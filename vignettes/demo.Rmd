...
To generate timeseries for the GLV, we need to specify the value of the control parameter $s$. Let's see what the GLV looks like for $s = .98$.
...
```
These dynamics can be better visualised in 3D phase space, where the underlying attractors becomes apparent. Click and move them around!
...
By changing $s$ gradually, we may produce *bifurcations*: qualitative changes in the dynamics due to small changes in the control parameter. Below we simulate a *subduction*: a sudden switch from periodic to chaotic behaviour.

```{r}
# Specify bifurcation parameter
bifpar_pars = list(
  bifpar_start = 1.01,
  bifpar_end = 1.02,
  pre_steps = 0,
