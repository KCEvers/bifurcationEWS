...
```{r Windowed EWS}
uni_metrics = c("mean"= mean, "variance"= var, 
...
noise_sigma = .01
noisy_df = df %>%
  add_obs_noise(X_names, noise_mean = 0, noise_sigma = noise_sigma, noise_constant = 0)
pl_noisy = ggplot(data = df %>% 
  dplyr::filter(bifpar_idx > 5, bifpar_idx < 7) %>%
  tidyr::gather(variable, value, -setdiff(colnames(.), X_names))) +
  geom_point(aes(x = time_idx, y = value), col = 'grey50', alpha = .75, size = .05) +
  geom_point(data = noisy_df %>% 
                 dplyr::filter(bifpar_idx > 5, bifpar_idx < 7) %>%
               tidyr::gather(variable, value, -setdiff(colnames(.), X_names)), 
