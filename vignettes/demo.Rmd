...
timestep = .01
nr_timesteps = 2000
bifpar_list = get_bifurcation_range(bifpar_start = .96, bifpar_end = .97, pre_steps = 0,
...
pl_peaks = peaks_df_ %>% ggplot() + geom_point(aes(x = bifpar_idx, y = X), size = .1) +
...
```{r intermittency, eval=FALSE, include=FALSE}
nr_timesteps = 100
bifpar_list = get_bifurcation_range(bifpar_start = .965, bifpar_end = .985, pre_steps = 0,
                                  baseline_steps = 0, transition_steps = 100, post_steps = 0)
...
pl_periods = ggplot(regime_list$period_per_var %>%
                        merge(peaks_df) %>%
...
# desired_switch = regime_list$regime_bounds[regime_list$regime_bounds$regime_switch == "Mixed-Periodic to Chaotic or Transitioning",]

desired_switch = regime_list$regime_bounds %>% dplyr::filter(regime_switch_type == "Mixed-Periodic to Chaotic or Transitioning" & regime1 == "Period-16 (X2,X3,X4) AND Period-20 (X1)" & regime2 == "Chaotic or Transitioning")

s_seq = get_bifurcation_range(bifpar_start = bifpar_list[[desired_switch$regime1_halfway_idx]]$s, 
                              bifpar_end = bifpar_list[[desired_switch$regime2_start_idx]]$s, 
                              pre_steps = pre_steps,
                              baseline_steps = baseline_steps, transition_steps = transition_steps, 
                              post_steps = post_steps)
X0 = unlist(desired_switch[,X_names])
               
# Generate data with changing bifurcation parameter
GLV_trans = bifurcation_ts(model = GLV_model, model_pars = model_pars,
               bifpar_list = s_seq,
               X0 = X0,
               timestep = timestep, nr_timesteps = nr_timesteps,
                downsample_pars = downsample_pars,
...
# Get dataframe with peaks
peaks_df_trans = peaks_bifdiag(GLV_trans$df, GLV_trans$X_names)
head(peaks_df_trans)

peaks_df_trans_ = peaks_df_trans %>% dplyr::filter(variable == "X1")
pl_peaks_trans = peaks_df_trans_ %>% ggplot() + geom_point(aes(x = bifpar_idx, y = X), size = .1) +
  labs(x = 'Bifurcation parameter', y = "X1", title = "Transient bifurcation diagram") +
  geom_vline(data = data.frame(xintercept = c(pre_steps + baseline_steps, 
                                              pre_steps + baseline_steps + transition_steps), 
                               moment = c("Baseline", "Transition") ),
             aes(xintercept = xintercept))

print(style_plot(pl_peaks_trans))

# Plot change in phase space
pl1 = rgl::plot3d(df_trans[sel_idxs,c("X1", "X2", "X3")], aspect = TRUE, lwd = 1, alpha = .01, main = sprintf("s = %.4f", model_pars[["s"]]))
...
pre_steps = pre_steps, baseline_steps = baseline_steps, transition_steps = transition_steps, post_steps = post_steps)

# Generate data with changing bifurcation parameter
GLV_null = bifurcation_ts(model = GLV_model, model_pars = model_pars,
               bifpar_list = s_seq,
               X0 = X0,
               timestep = timestep, nr_timesteps = nr_timesteps,
                downsample_pars = downsample_pars,
...
# We need to check whether our simulation generated the desired regimes using the same process as above:
# Get dataframe with peaks
peaks_df_null = peaks_bifdiag(GLV_null$df, GLV_null$X_names)
head(peaks_df_null)


peaks_df_null_ = peaks_df_null %>% dplyr::filter(variable == "X1")
pl_peaks_null = peaks_df_null_ %>% ggplot() + geom_point(aes(x = bifpar_idx, y = X), size = .1) +
  labs(x = 'Bifurcation parameter', y = "X1", title = "Transient bifurcation diagram") +
  geom_vline(data = data.frame(xintercept = c(pre_steps + baseline_steps, 
                                              pre_steps + baseline_steps + transition_steps), 
                               moment = c("Baseline", "Transition") ),
...
print(style_plot(pl_peaks_null))
```
...
# label_func = function(x){ 
  # labels = data.frame(var = "spatial_variance", label = "Spatial Variance")
  # return(labels[labels$var == x, ]$label)
# }
