...
To generate timeseries for the Generalised Lotka-Volterra (GLV) model, we need to specify the value of the control parameter $s$. Let's see what the GLV looks like for $s = .98$.
...
We can repeat this procedure for different values of $s$ to get an idea of the dynamics the GLV is capable of.
...
pl_ts = GLV_dfs %>% 
  ...
  geom_point(aes(x = time_idx, y = X1, color = factor(s)), size = .1) +
    labs(x = 'Time', y = expression(X[1]), title = "Timeseries") + 
  facet_wrap(s ~ ., ncol = 2) +
...
```{r 3D phase space}
landscape_GLVs = plot_3D_landscape_trans(GLV_dfs %>% filter(time > nr_timesteps*.25),
...
By changing $s$ gradually, we may produce *bifurcations*: qualitative changes in system dynamics due to small changes in the control parameter. Below we generate a *subduction*: a sudden switch from periodic to chaotic behaviour.
...
GLV = bifurcation_ts(model = pars$model, model_pars = pars$model_pars,
                 ...
                 X_names = pars$X_names,
                   fs = pars$fs,
                 seed_nr = round(s*100), silent = F
                 )

# Plot 3D landscape
sel_bifpar_idxs = round(c(.1, .15, .2, .25, .3, 1) * length(GLV$bifpar_list)) 
landscape_GLVs = plot_3D_landscape_trans(GLV$df %>% 
                                           filter(time > nr_timesteps*.25),
                                         bifpar_idxs = sel_bifpar_idxs,
                                         bifpar_values = purrr::map(GLV$bifpar_list[sel_bifpar_idxs], "s"), 
...
Small variations in the initial conditions can create differences in the timing of the bifurcation.
...
for (seed in seeds){
  ...
  GLV = bifurcation_ts(model = pars$model, model_pars = pars$model_pars,
                 ...
                 X_names = pars$X_names,
                 fs = pars$fs,
                 seed_nr = seed, silent = F
                 )
  GLVs[[seed]] = GLV$df %>% dplyr::mutate(s = seed, bifpar_idx = seed)
...
GLV_dfs = foreach::foreach(seed = seeds, .combine = 'rbind', .packages = c("dplyr",  "bifurcationEWS")){
  ...
  return(GLV$df %>% dplyr::mutate(s = seed, bifpar_idx = seed))
...
### Bifurcation diagram
To visualise these changes, we can also use *bifurcation diagram*, which shows the peaks of the timeseries for each value of the control parameter.
