...
timestep = .01
nr_timesteps = 2000
bifpar_list = get_bifurcation_range(bifpar_start = .96, bifpar_end = .97, #bifpar_start = .965, bifpar_end = .985,
                                    pre_steps = 0,
...
regimes_full = find_regimes(GLV_full,
                             ...
                             nr_smooth = 0,
                              factor_k = .1)
print(head(regimes_full$period_per_var))
print(regimes_full$periods)
...
desired_switch = regimes_full$regime_bounds %>% 
  ...
  dplyr::filter(regime1 == "Period-4 (X2,X3,X4) AND Period-6 (X1)" & regime2 == "Period-10 (X1) AND Period-8 (X2,X3,X4)")

bifpar_pars = list(pre_steps = 50,
baseline_steps = 50,
transition_steps = 50,
post_steps = 50,
...
for (trans_or_null in c("transition", "null")){
  if (trans_or_null == "transition"){
    bifpar_pars_ = bifpar_pars
  } else if (trans_or_null == "null"){
    bifpar_pars_ = utils::modifyList(bifpar_pars, list(bifpar_end = NA))
  }
  ...
  # Get dataframe with peaks
  peaks_df_trans = peaks_bifdiag(GLV$df, GLV$X_names)
  head(peaks_df_trans)
  ...
  regimes_trans = find_regimes(GLV,
                               thresh_node = .001,
                               thresh_coord_spread = .025,
                               thresh_peak_idx_spread=2,
                               min_length_regime =5,
                               nr_smooth = 0,
                                factor_k = .1)
  print(head(regimes_trans$period_per_var))
  print(regimes_trans$periods)
  print(regimes_trans$regimes)
  print(regimes_trans$regime_bounds)
  
  # Plot change in phase space
  plot_landscape(GLV$df, GLV$bifpar_list, bifpar_idx1 = pre_steps + baseline_steps, 
